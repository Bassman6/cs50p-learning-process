...
#  prompt the user again. Assume that every month has no more than 31 days; no need to validate whether a
...
while True:
    ...
    if "/" in date:
        ...
        if "," in date:
            ...
            try:
                # date = date.replace(",", "")
                m_str, d_str, y_str = date.replace(",", "").split(" ")

                y, m, d = int(y_str), z.index(m_str), int(d_str)
